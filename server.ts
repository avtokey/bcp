...
const dbPath = path.join(__dirname, "bcp.db");
...
const statusSeedDone = db.prepare("SELECT value FROM app_metadata WHERE key = 'status_seed_v2'").get();
if (!statusSeedDone) {
  const updateStatus = db.prepare("UPDATE staff SET remote_access = ?, problem_type = ? WHERE name = ? AND staff_info = ?");
  ...
  const updateTransaction = db.transaction(() => {
    for (const [name, info, remote, prob] of statusUpdates) {
      updateStatus.run(remote, prob, name, info);
    }
    db.prepare("INSERT OR REPLACE INTO app_metadata (key, value) VALUES ('status_seed_v2', 'true')").run();
    db.prepare("INSERT OR REPLACE INTO app_metadata (key, value) VALUES ('last_global_update', ?)").run(new Date().toISOString());
  });
...
async function startServer() {
  ...
  app.get("/api/download-db", (req, res) => {
    res.download(dbPath, "bcp.db");
  });

  app.post("/api/restore-db", upload.single('database'), (req: any, res) => {
    if (!req.file) {
